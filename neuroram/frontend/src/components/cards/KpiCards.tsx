...
export type KpiCardKey = "ram" | "predicted" | "stability" | "devices";

type Props = {
  payload: DashboardPayload;
  activeCard?: KpiCardKey;
  onCardClick?: (card: KpiCardKey) => void;
};

export function KpiCards({ payload, activeCard, onCardClick }: Props) {
  ...
  return (
    <section className="kpi-grid">
      {cards.map((card) => {
        ...
        return (
          <article
            className={`kpi-card ${onCardClick ? "kpi-card--interactive" : ""} ${activeCard === card.key ? "kpi-card--active" : ""}`}
            key={card.label}
